import{M as ya,b as wa,T as Aa}from"./bootstrap-CeGnflBh.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))t(n);new MutationObserver(n=>{for(const s of n)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&t(o)}).observe(document,{childList:!0,subtree:!0});function i(n){const s={};return n.integrity&&(s.integrity=n.integrity),n.referrerPolicy&&(s.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?s.credentials="include":n.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function t(n){if(n.ep)return;n.ep=!0;const s=i(n);fetch(n.href,s)}})();const Sa={"/":"view-landing","/home":"view-landing","/form":"view-form","/preview":"view-preview","/result":"view-result","/draft":"view-draft","/history":"view-history","/doa":"view-doa","/settings":"view-settings"};function aa(){return window.location.hash.slice(1)||"/"}function Ta(a){document.querySelectorAll(".view-section").forEach(t=>{t.classList.add("d-none")});const i=document.getElementById(a);if(i){i.classList.remove("d-none"),window.scrollTo(0,0);const t=new CustomEvent("viewChanged",{detail:{viewId:a,path:aa()}});window.dispatchEvent(t)}else console.error(`View with ID "${a}" not found`)}function Ea(a){document.querySelectorAll(".navbar-nav .nav-link").forEach(i=>{const t=i.getAttribute("href");i.classList.remove("active"),(t===`#${a}`||a==="/"&&t==="#/")&&i.classList.add("active")})}function O(){const a=aa(),e=Sa[a];e?(Ta(e),Ea(a)):(console.warn(`Route "${a}" not found, redirecting to home`),v("/"))}function v(a){window.location.hash=`#${a}`}function Da(){O(),window.addEventListener("hashchange",O),document.addEventListener("click",a=>{const e=a.target.closest('a[href^="#/"]');if(e){a.preventDefault();const i=e.getAttribute("href").slice(1);v(i)}})}let E=null,D=null,P=null;function Pa(){return q(),E=a=>{a.key==="Escape"&&La()},D=a=>{Ia(a.target)},P=()=>{Ca()},document.addEventListener("keydown",E),document.addEventListener("show.bs.modal",D),document.addEventListener("hide.bs.modal",P),q}function q(){E&&(document.removeEventListener("keydown",E),E=null),D&&(document.removeEventListener("show.bs.modal",D),D=null),P&&(document.removeEventListener("hide.bs.modal",P),P=null)}function La(){const a=document.querySelector(".modal.show");if(a){const e=ya.getInstance(a);e&&e.hide()}}const K=new WeakMap;function Ia(a){if(!a)return;if(K.has(a)){const s=K.get(a);a.removeEventListener("keydown",s)}const e=a.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'),i=e[0],t=e[e.length-1];a.dataset.previousFocus=document.activeElement?.id||"",setTimeout(()=>{if(i)try{i.focus()}catch{}},100);const n=s=>{if(s.key==="Tab"){if(s.shiftKey){if(document.activeElement===i){s.preventDefault();try{t.focus()}catch{}}}else if(document.activeElement===t){s.preventDefault();try{i.focus()}catch{}}}};K.set(a,n),a.addEventListener("keydown",n)}function Ca(){const a=document.querySelector(".modal");if(a&&a.dataset.previousFocus){const e=document.getElementById(a.dataset.previousFocus);if(e){const i=e.hasAttribute("disabled"),t=e.offsetParent===null;if(!i&&!t)try{e.focus()}catch{}}}}function xa(){document.addEventListener("keydown",a=>{if((a.ctrlKey||a.metaKey)&&a.key==="s"){a.preventDefault();const e=document.getElementById("btn-save-draft");e&&!e.disabled&&e.click()}if((a.ctrlKey||a.metaKey)&&a.key==="Enter"){const e=document.querySelector('button[type="submit"]');e&&!e.disabled&&e.click()}})}function ja(){const a=document.getElementById("view-landing");a.innerHTML=`
    <!-- Hero Section -->
  ...
  `;u.insertAdjacentHTML("beforeend",w);const p=document.getElementById(b);if(!p){console.error("[ToastNotification] Failed to create toast element");return}const V=new bootstrap.Toast(p,{autohide:o>0,delay:o});p.addEventListener("hidden.bs.toast",()=>{p.remove(),typeof l=="function"&&l()}),V.show(),console.log(`[ToastNotification] Showed ${s} toast: ${t}`)},me=a=>Y[a]||Y.info,be="spinner-border-sm",x="originalContent",j="originalDisabled";function C(a,e="Memproses..."){a&&(a.dataset[x]=a.innerHTML,a.dataset[j]=a.disabled,a.disabled=!0,a.innerHTML=`
...
`)}catch(e){throw console.error("[cvGeneratorService] Error generating CV:",e),e}};function Me(){const a=document.getElementById("view-preview"),e=ca();if(Object.keys(e).length===0){d("Tidak ada data untuk di-preview. Silakan isi formulir terlebih dahulu.","warning"),v("/form");return}const i=He(e);a.innerHTML=`
    <!-- Preview Header -->
    <div class="mb-4">
      <h2>
    ...
    <!-- Spacing for sticky action bar -->
    <div style="height: 80px;"></div>
  `,Ve()}function ca(){try{const a=sessionStorage.getItem(h.TEMP_DATA);return a?JSON.parse(a):{}}catch(a){return console.error("[PreviewView] Error loading form data:",a),{}}}function He(a){let e="";const i=t=>t||"-";return e+=`
  ...
  `,ua()}function ua(){const a=document.getElementById("draft-list-container"),i=N().sort((t,n)=>new Date(n.lastUpdated)-new Date(t.lastUpdated));i.length===0?We(a):Ue(a,i),console.log(`[DraftView] Loaded ${i.length} drafts`)}function We(a){a.innerHTML=`
    <div class="card">
...
Tindakan ini tidak dapat dibatalkan.`;if(!confirm(i))return;const t=ee(a);t.success?(d("Draft berhasil dihapus","success"),console.log("[DraftView] Deleted draft:",a),ua()):(d(t.message||"Gagal menghapus draft","error"),console.error("[DraftView] Delete failed:",t))}function ze(){const a=document.getElementById("draft-list-container");if(!a)return;a.addEventListener("click",i=>{const t=i.target.closest(".btn-load-draft");if(t){const s=t.getAttribute("data-id");Ge(s,t);return}const n=i.target.closest(".btn-delete-draft");if(n){const s=n.getAttribute("data-id");Ye(s);return}}),[].slice.call(a.querySelectorAll('[data-bs-toggle="tooltip"]')).forEach(i=>{new bootstrap.Tooltip(i)})}function Qe(){Je()}window.addEventListener("viewChanged",a=>{a.detail.viewId==="view-draft"&&Qe()});function Xe(){const a=document.getElementById("view-history");a.innerHTML=`
  ...
  `,ma()}function ma(){const a=document.getElementById("history-list-container"),i=_().sort((t,n)=>new Date(n.generatedAt)-new Date(t.generatedAt));i.length===0?Ze(a):ai(a,i),console.log(`[HistoryView] Loaded ${i.length} history items`)}function Ze(a){a.innerHTML=`
    <div class="card">
      <div class="card-body empty-state">
...
Sumber: ${s}`,p=T(w);return`
    <div class="card ${ci}" ${X}="${e}" ${mi}="${o}">
      ...
      <div class="card-footer">
        <button 
          class="btn btn-outline-primary btn-sm ${pa}" 
          ${bi}="${p}"
          ${X}="${e}"
        >
          <i class="bi bi-clipboard"></i> Copy
